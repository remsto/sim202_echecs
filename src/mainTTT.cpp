...
int main2() {
  ...
  while (num_tour < boucle_max && !is_fini) {
    ...
    if (joueur_courant == 1) {
      ...
      coupjoue = new Coup(is_white_courant, *piece_jouee, p,
                          piece_jouee->position_coor, num_tour);
    ...
    // C'est à l'ordi aléatoire de jouer, , choix du coup !
      ...
      cout << "Au tour " << num_tour << ", le coup du joueur "
           << (is_white_courant ? "blanc" : "noir") << " est : " << *coupjoue
           << endl;
    }
