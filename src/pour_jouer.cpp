...
Coup::Coup(bool isW, const Piece &pieceJ, pair<int, int> newP,
           pair<int, int> oldP, int num_tour, Piece *taken, bool isspecial,
           Coup *next, Coup *prev, bool is_ech, bool is_m) {
  isWhite = isW;
...
ListeCoups *coupsPossiblesTTT(const Echiquier &plateau, bool isWhite,
                              int num_tour) // est-ce au joueur blanc de jouer ?
{
  int nbC = 0;
  ...
  for (int i = 1; i <= 3; i++) {
    for (int j = 1; j <= 3; j++) {
      if (plateau.plateau[coor_to_pos(pair<int, int>(i, j), taillep)] == NULL) {
        if (nbC == 0) {
          first = new Coup(isWhite, Piece(isWhite), pair<int, int>(i, j),
                           pair<int, int>(0, 0), num_tour);
          prev = first;
        } else {
          current = new Coup(isWhite, Piece(isWhite), pair<int, int>(i, j),
                             pair<int, int>(0, 0), num_tour);
          prev->Next = current;
          prev = current;
        }
...
ListeCoups *coupsPossibles(Echiquier &plateau, bool isWhit, int num_tour) {
  ...
  for (int i = 1; i <= taillep; i++) {
    for (int j = 1; j <= taillep; j++) {
      Piece *piece_a_jouer =
          plateau.plateau[coor_to_pos(pair<int, int>(i, j), taillep)];
        ...
        while (dep_current != NULL) {
          if (is_legal(plateau, piece_a_jouer, dep_current, num_tour)) {
            if (nbC == 0) {
              ...
              first = new Coup(
                  isWhit, Piece(*piece_a_jouer),
                  dep_current->coor + piece_a_jouer->position_coor,
                  piece_a_jouer->position_coor, num_tour,
                  taken_coup(plateau, piece_a_jouer, dep_current, num_tour),
                  is_Special(plateau, piece_a_jouer, dep_current), NULL, NULL,
                  is_Echec(plateau, piece_a_jouer, dep_current, num_tour,
                           !isWhit),
                  is_Mat(plateau, piece_a_jouer, dep_current, num_tour));
              prev = first;
            } else {
              ...
              current = new Coup(
                  ...
                  piece_a_jouer->position_coor, num_tour,
                  taken_coup(plateau, piece_a_jouer, dep_current, num_tour),
                  is_Special(plateau, piece_a_jouer, dep_current), NULL, prev,
                  is_Echec(plateau, piece_a_jouer, dep_current, num_tour,
                           !isWhit),
                  is_Mat(plateau, piece_a_jouer, dep_current, num_tour));
              prev->Next = current;
...
ListeCoups::ListeCoups(int nbC, Coup *first_acopier, Coup *last_acopier) {
  ...
  // Coup *last_copier = new Coup(last_acopier);  // je pense qu'il ne faut pas
  // les copier ! A vérifier !
  first = first_acopier;
  last = last_acopier;
}
...
ostream &operator<<(ostream &out, const ListeCoups &Listcoup) {
  ...
  while (current_Coups != NULL) {
    current_Coups->isWhite ? out << current_Coups->num_tour_de_jeu << "."
                           : out << "";
    out << " ";
    out << *current_Coups;
...
void actualisePlateau(Echiquier &plateau, const Coup &coupjoue) {
  ...
  {
    Piece *new_piece =
        new Piece(coupjoue.pieceJouee); // on doit créer la pièce ?
    new_piece->position_coor = news;
    plateau.plateau[coor_to_pos(news, taillep)] = new_piece;
...
void resetPlateau(Echiquier &plateau, const ListeCoups &coupsPrecedents) {
  ...
  while (coup != NULL) {
    ...
    pair<int, int> old = coup->oldPosition;
    pair<int, int> news = coup->newPosition;
    Piece *new_piece = plateau.plateau[coor_to_pos(
        news, taillep)]; // on remonte à l'envers les étapes

    if (taillep == 3) // si tictactoe
    {
      new_piece->~Piece(); // on doit supprimer la pièce !
    } else // si on joue aux echecs ! faire la prise ! A REVOIR ! impossible ou
           // rajouter la liste des pièces prises
    {
      Piece *old_piece = plateau.plateau[coor_to_pos(old, taillep)];
      old_piece = new_piece;
      new_piece = NULL;
...
bool is_legal(Echiquier &plateau, Piece *piece_a_jouer, Deplac_rel *dep_current,
              int num_tour) // chiant
{
  int taillep = plateau.taille;
  ...
  if (piece_a_jouer->type == "Pion") {
    ...
    if (((dep_current->coor == pair<int, int>(1, 0)) ||
    ...
    if (((dep_current->coor == pair<int, int>(1, 1)) ||
...
bool is_piece_entre(const Echiquier &plateau, pair<int, int> place_depart,
                    pair<int, int> place_arrive) {
  int taillep = plateau.taille;
  ...
  } else if ((roi_i - ki) * (roi_j - kj) > 0) // diagonale aigue
  {
    int m = 1;
    for (int l = min(kj, roi_j) + 1; l <= max(kj, roi_j) - 1; l++) {
      if (plateau.plateau[coor_to_pos(pair<int, int>(min(ki, roi_i) + m, l),
                                      taillep)] != NULL)
        return true;
      m += 1;
    }
  } else // diagonale grave  (1,6)-->(4,3) l=4,5
  {
    int n = 1;
    for (int l = min(kj, roi_j) + 1; l <= max(kj, roi_j) - 1; l++) {
      if (plateau.plateau[coor_to_pos(pair<int, int>(max(ki, roi_i) - n, l),
                                      taillep)] != NULL)
        return true;
      n += 1;
    }
...
Piece *taken_coup(Echiquier &plateau, Piece *piece_a_jouer,
                  Deplac_rel *dep_current, int num_tour) {

  int taille = plateau.taille;
  bool couleur_joueur = piece_a_jouer->isWhite;
  pair<int, int> coor_arriver =
      piece_a_jouer->position_coor + dep_current->coor;
...
bool is_Special(const Echiquier &plateau, Piece *piece_a_jouer,
                Deplac_rel *dep_current) {
  return false;
}

bool is_Echec(Echiquier &plateau, Piece *piece_a_jouer, Deplac_rel *dep_current,
              int num_tour, bool couleur_roi_en_echec) {
  // on ne vérifie ^pas la légalité, supposé fait avant
  int taillep = plateau.taille;
  bool couleur_joueur = piece_a_jouer->isWhite;

  // on doit verifier si l'un des déplacement de la pièce menace le roi ennemi
  // sans piece entre les deux et si une autre piece vient menacer le roi !
  pair<int, int> new_position =
      piece_a_jouer->position_coor + dep_current->coor;
  Coup coup_jouee = Coup(
      couleur_joueur, piece_a_jouer, new_position, piece_a_jouer->position_coor,
      num_tour, taken_coup(plateau, piece_a_jouer, dep_current, num_tour),
      is_Special(plateau, piece_a_jouer, dep_current), NULL, NULL, false,
      false); // a voir pour les deux derniers
  actualisePlateau(plateau, coup_jouee);
  pair<int, int> place_roi_ennemi =
      (couleur_roi_en_echec ? plateau.roi_blanc->position_coor
                            : plateau.roi_noir->position_coor);
  // plateau.affiche();

  // on parcourt toutes les pièces du joueur  contre le roi. peut etre changer
  // le sens de parcours i et j pour optimalité
  for (int i = 1; i <= taillep; i++) {
    for (int j = 1; j <= taillep; j++) {
      Piece *piece_a_jouer_bis =
          plateau.plateau[coor_to_pos(pair<int, int>(i, j), taillep)];
      if ((piece_a_jouer_bis != NULL) &&
          (piece_a_jouer_bis->isWhite != couleur_roi_en_echec)) {
        ...
        while (dep_current_bis != NULL) {
          if ((place_roi_ennemi ==
               (piece_a_jouer_bis->position_coor + dep_current_bis->coor)) &&
              (is_legal(plateau, piece_a_jouer_bis, dep_current_bis,
                        num_tour))) {
...
bool is_Mat(Echiquier &plateau, Piece *piece_a_jouer, Deplac_rel *dep_current,
            int num_tour) {
  bool couleur_joueur = piece_a_jouer->isWhite;
  if (!is_Echec(plateau, piece_a_jouer, dep_current, num_tour,
                !couleur_joueur)) // pas opti !
    return false;

  int taillep = plateau.taille;
  Piece *roi_ennemi =
      ((!couleur_joueur) ? plateau.roi_blanc : plateau.roi_noir);
  ...
  //  check si une pièce ennemi autre le roi peut se mettre enstre les pièces ou
  //  une piece peut graille la pièce = si le roi est toujours en echec
  for (int i = 1; i <= taillep; i++) {
    for (int j = 1; j <= taillep; j++) {
      Piece *piece_pouvant_defendre =
          plateau.plateau[coor_to_pos(pair<int, int>(i, j), taillep)];
      if (piece_pouvant_defendre != NULL &&
          piece_pouvant_defendre->isWhite == couleur_roi_ennemi) {
        ListDeplac_rel Listdep_piece_pouvant_defendre =
            piece_pouvant_defendre->deplac_relatif;
        Deplac_rel *dep_piece_pouvant_defendre =
            Listdep_piece_pouvant_defendre.first;
        while (dep_piece_pouvant_defendre != NULL) {

          if (is_legal(
                  plateau, piece_pouvant_defendre, dep_piece_pouvant_defendre,
                  num_tour +
                      1)) // && (!is_Echec(plateau, piece_pouvant_defendre,
                          // dep_piece_pouvant_defendre, num_tour + 1,
                          // couleur_roi_ennemi))) DEJA TEST DANS IS LEGAL !!!
          {
            resetPlateau(plateau, coup_jouee);
            cout << "la piece pouvant defendre est : "
                 << *piece_pouvant_defendre << " en se déplacant ainsi "
                 << *dep_piece_pouvant_defendre << "a partir de "
                 << piece_pouvant_defendre->position_coor;
            return false;
          }
