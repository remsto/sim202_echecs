...
bool Position::estGagnante(){
    ...
    for(int j=1;j<=3;j++){
        if (plateau[(i-1)*taille + (j-1)]!=Null{
            if(plateau[(i-1)*taille + (j-1)]->isWhite==couleur) p++;
        }
    }
    if (p==3) return True;
    ...
    for(int i=1;i<=3;i++){
        if (plateau[(i-1)*taille + (colonne-1)]!=Null{
            if(plateau[(i-1)*taille + (colonne-1)]->isWhite==couleur) q++;
        }
    }
    if (q==3) return True;
    ...
    if(ligne==colonne){
        ...
        for(int k=1;k<=3;k++){
            if (plateau[(k-1)*taille + (k-1)]!=Null{
                if(plateau[(k-1)*taille + (k-1)]->isWhite==couleur) p++;
            }
            if (plateau[(3-k)*taille + (3-k)]!=Null{
                if(plateau[(3-k)*taille + (3-k)]->isWhite==couleur) q++;
            }
        }
    }

    if ((p==3)||(q==3)) return True;
    return False;
